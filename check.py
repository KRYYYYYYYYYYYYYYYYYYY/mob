...
# --- ЦИКЛ ПРОВЕРКИ (ИЩЕМ 200 РАБОЧИХ) ---
    ...
    # 1. Сначала распределяем всё, что попало в working_for_sub, по категориям
    ...
    final_to_sub = []
    seen_in_final = set() # Сито, чтобы не было дублей по IP/базе
    
    # 2. ПРИОРЕТЕТ №0: Избранные (⭐) всегда в самом верху
    ...
    # 3. ПРИОРЕТЕТ №1: Закрепы (💎) - Лимит 130 мест (вместе с избранными)
    for l in all_pinned:
    ...
    # 4. ПРИОРЕТЕТ №2: Добираем обычные (mob), пока не наберем 200
    for l in all_others:
        if len(final_to_sub) >= 200: break
        base = l.split("#")[0].strip()
        if base not in seen_in_final:
    ...
    # 5. Формируем deferred.txt (те, кто не влез в 200)
    # Берем хвост очереди + тех, кто прошел проверку, но не попал в лимит final_to_sub
    ...
    # Сохраняем очередь на потом
    with open('test1/deferred.txt', "w", encoding="utf-8") as f:
        f.write("\n".join(deferred_final))
    
    # Формируем и сохраняем основную подписку
    final_content = HEADER.strip() + "\n\n" + "\n".join(final_to_sub)
    os.makedirs(os.path.dirname(OUTPUT_FILE), exist_ok=True)
    with open(OUTPUT_FILE, "w", encoding="utf-8") as f:
        f.write(final_content)
        
    # Сохраняем рабочую базу (только mob части) для 1.txt
    os.makedirs(os.path.dirname(INPUT_FILE), exist_ok=True)
    with open(INPUT_FILE, "w", encoding="utf-8") as f: 
        f.write("\n".join(working_for_base))
    
    # Отчет в логах
    print(f"🏁 План выполнен: {len(final_to_sub)} в подписке. Остаток в очереди: {len(deferred_final)}")
